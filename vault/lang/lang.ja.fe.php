 ...
 * @see LICENSE.txt
 *
 * This file: Japanese language data for the front-end (last modified: 2016.12.19).
 */
...
$phpMussel['lang']['config_general_honeypot_mode'] = 'ハニーポットモードが有効になっていると、phpMusselはアップロードされてきた全てのファイルを例外なく検疫します。署名にマッチするかどうかは問題としません。 スキャニングや分析もなされません。 phpMusselをウィルス／マルウェアのリサーチに利用と考えているユーザーにとって有益と言えるでしょう。ただし、アップロードファイルのスキャニングという点からは、あまり推奨できませんし、ハニーポット・モードを本来の目的以外に使用することもお勧めできません。 デフォルト設定では無効です。 `false`（偽） = Disabled（無効） 「Default（デフォルト）」； `true`（真） = Enabled（有効）。';
...
$phpMussel['lang']['config_heuristic_threshold'] = 'phpMusselには、このファイルは疑わしく危険性が高いと判断する署名があります。 しきい値は、アップロードされているファイルの危険性の最大値であり、これを超えるとマルウェアと判断されます。ここにおける危険性の定義とは、疑わしいと特定されたものの総数です。 デフォルトでは３に設定されています。 これより低いと誤検出の可能性が増え、大きすぎると、誤検出は減るものの危険性のあるファイルが検出されない可能性が増加してしまいます。 特に問題がなければ、デフォルト値のままにしておくことお勧めします。';
$phpMussel['lang']['config_signatures_detect_adware'] = 'phpMusselはアドウェア検出のために署名を分析すべきか否か？ `false`（偽） = いいえ; `true`（真） = はい 「Default（デフォルト）」';
$phpMussel['lang']['config_signatures_detect_deface'] = 'phpMusselは改ざんやディフェーサー検出のために署名を分析すべきか否か？ `false`（偽） = いいえ; `true`（真） = はい 「Default（デフォルト）」';
$phpMussel['lang']['config_signatures_detect_joke_hoax'] = 'phpMusselは悪戯／偽造やマルウェア／ウィルス検出のために署名を分析すべきか否か？ `false`（偽） = いいえ; `true`（真） = はい 「Default（デフォルト）」';
$phpMussel['lang']['config_signatures_detect_packer_packed'] = 'phpMusselはパッカーやパックデータ検出のために署名を分析すべきか否か？ `false`（偽） = いいえ; `true`（真） = はい 「Default（デフォルト）」';
$phpMussel['lang']['config_signatures_detect_pua_pup'] = 'phpMusselはPUAs/PUPs検出のために署名を分析すべきか否か？ `false`（偽） = いいえ; `true`（真） = はい 「Default（デフォルト）」';
$phpMussel['lang']['config_signatures_detect_shell'] = 'phpMusselはshellスクリプト検出のために署名を分析すべきか否か？ `false`（偽） = いいえ; `true`（真） = はい 「Default（デフォルト）」';
...
$phpMussel['lang']['config_signatures_fail_silently'] = '署名ファイルがない、あるいは破損している場合に、phpMusselがそれをリポートすべきか否か？ `fail_silently`が無効ならば、問題はリポートされ、有効であれば、問題は無視されたスキャニングレポートが作成されます。 クラッシュするというような害がなければ、デフォルト設定のままにしておくべきです。 `false`（偽） = Disabled（無効）; `true`（真） = Enabled（有効） 「Default（デフォルト）」';
$phpMussel['lang']['config_signatures_MD5'] = 'スキャニング時にＭＤ５署名に対してチェックしますか？ その場合は、使用するシグネチャファイルの名前をコンマで区切って指定します。';
$phpMussel['lang']['config_signatures_PE_Extended'] = 'スキャニング時にＰＥ（ポータブル・エグゼキュータブル）ファイル（ＥＸＥ、ＤＬＬ、等）をＰＥ拡張署名に対してチェックしますか？ その場合は、使用するシグネチャファイルの名前をコンマで区切って指定します。';
$phpMussel['lang']['config_signatures_PE_Sectional'] = 'スキャニング時にＰＥ（ポータブル・エグゼキュータブル）ファイル（ＥＸＥ、ＤＬＬ、等）をＰＥセクショナル署名に対してチェックしますか？ その場合は、使用するシグネチャファイルの名前をコンマで区切って指定します。';
